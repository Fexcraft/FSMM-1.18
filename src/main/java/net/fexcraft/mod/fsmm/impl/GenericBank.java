...
public class GenericBank extends Bank {
			...
			case DEPOSIT:{
				...
				if(receiver.getBalance() + amount <= Long.MAX_VALUE){
					...
					if(ItemManager.countInInventory(player) - total >= 0){
						...
						FSMM.LOGGER.error(receiver.getAsResourceLocation().toString() + ": Deposit failed! Not enough money in Inventory. (D:" + amount + " || B:" + receiver.getBalance() + ");");
						return false;
					}
				}
				Print.chat(player, "Deposit failed! Result is above technical limit. (D:" + amount + " || B:" + receiver.getBalance() + ");");
				FSMM.LOGGER.error(receiver.getAsResourceLocation().toString() + " : Deposit failed! Result is above technical limit. (D:" + amount + " || B:" + receiver.getBalance() + ");");
				return false;
			}
			case TRANSFER:{
				if(sender == null){
	...
	public String getName(CommandSource sender){
		return sender == null ? "[NULL]" : sender instanceof Player ? ((Player)sender).getName().getString() : "[SENDER]";
	}
